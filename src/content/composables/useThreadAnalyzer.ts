...
export function useThreadAnalyzer() {
  ...
  const analyzeThread = async (): Promise<AnalysisResult> => {
    ...
    try {
      ...
      // Log debug info to content script console (visible in page DevTools)
      console.log('[X Thread Analyzer] Analysis result received:')
      ...
      console.log('  Categories:', result.categories?.map(c => `${c.name} (${c.comments?.length || 0})`).join(', '))
